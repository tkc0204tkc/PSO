...
class particle_multi(particle_single):
    ...
    def __init__(self,obj_func,attribute_number,constr=[],vmax=np.array(np.nan),l_bound=np.array(np.nan),u_bound=np.array(np.nan),integer=np.array(np.nan),position=np.array(np.nan), velocity=np.array(np.nan), init_method = 0, extreme_asset = 0):
        ...
        try:
            if init_method == 0:
                self.init_position_to_random(attribute_number, l_bound, u_bound, integer, position)
                # print(f"position : {self.position}")
                # print(f"obj values: {self.calc_obj_value()}")
            elif init_method == 1:
                self.init_position_to_average(attribute_number, l_bound, u_bound, position)
                # print(f"position : {self.position}")
                # print(f"obj values: {self.calc_obj_value()}")
            elif init_method == 2:
                self.init_position_to_extreme(attribute_number, position, extreme_asset)
                # print(f"position : {self.position}")
                # print(f"obj values: {self.calc_obj_value()}")
        except:
    ...
    def init_position_to_random(self, attribute_number, l_bound, u_bound, integer, position):
        if np.all(np.isnan(position))==False:
            self.position=position
        else:
            try:
    ...
    def init_position_to_average(self, attribute_number, l_bound, u_bound, position):
        if np.all(np.isnan(position))==False:
            self.position=position
        else:
            try:
    ...
    def init_position_to_extreme(self, attribute_number, position, extreme_asset = 0):
        if np.all(np.isnan(position))==False:
            self.position=position
        else:
            try:
                if attribute_number!=1:
                    init_pos = [0 for i in range(self.att)]
                    init_pos[extreme_asset] = 1
                    self.position = np.array(init_pos)
                else:
                    self.position= [1]
            except:
